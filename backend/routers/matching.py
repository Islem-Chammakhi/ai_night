...
from config import TOP_K_EMBEDDING, TOP_K_FINAL, WEIGHT_EMBEDDING, WEIGHT_RERANKER, WEIGHT_SKILL
...
def match_cvs(request: MatchRequest, db: Session = Depends(get_db)):
    ...
    if total_cvs == 0:
        raise HTTPException(status_code=400, detail="No CVs in the system. Please upload CVs first.")

    # --- JUDGE 1: Embedding Search ---
    top_matches = search_similar_cvs(request.requirements, top_k=TOP_K_EMBEDDING)
    if not top_matches:
    ...
    cvs_map = {cv.id: cv for cv in db.query(CV).filter(CV.id.in_(cv_ids)).all()}
